...
class neatNN(object):
    ...
    def evaluate(self, genome):
        ...
        while (not winner):
            ...
            if (not 1 in winner):  # if there is not a winner
                genome.fitness += 1  # Increment fitness
    ...
    def breedingControl(self, species, percentKept): 
        ...
        return nextGenerationUnorted # An unsorted list of our next   
...
class genome(object):
    ...
    def __init__(self, parent, initialInputs, initialOutputs, initialConnections): 
        ...
        for i in xrange(initialConnections):
            self.addConnection(random.randint(-1, len(initialInputs) - 1), random.randint(len(initialInputs), len(initialInputs) + len(initialOutputs) - 1), parent.innovation, random.uniform(-1, 1))
            parent.innovation += 1
    ...
    def addConnection(self, inNode, outNode, innovation, weight):
        ...
        if inNode == -1:
            self.addNode(inputNode(-1, self, (-1,-1)))
        ...
        self.nodes[inNode].refresh()
        self.nodes[outNode].refresh()
    ...
    def mutateAddNode(self, parent):
        ...
        self.addConnecton(g.inNode, self.maxNodeNumber, parent.innovation, 1) #
        parent.innovation += 1
        self.addConection(self.maxNodeNumber, g.outNode, parent.innovation, g.weight)
        parent.innovation += 1
        self.addNode(hiddenNode(self.maxNodeNumber + 1, self)) # Adds a new hidden node
